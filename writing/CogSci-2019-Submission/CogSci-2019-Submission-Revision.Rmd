...
author-information: > 
    \author{{\large \bf Bria Long} \\ Department of Psychology \\ Stanford University \\ 450 Serra Mall \\ Stanford, CA 94305 \\ \textit{bria@stanford.edu}
    \And {\large \bf Judith E. Fan} \\ Department of Psychology  \\ UC San Diego  \\  9500 Gilman Drive \\ La Jolla, CA 92093 \\ \textit{jefan@ucsd.edu}
    \And {\large \bf Zixian Chai} \\ Department of Psychology  \\  Stanford University \\  450 Serra Mall \\ Stanford, CA 94305 \\ \textit{zchai14@stanford.edu}
    \And {\large \bf Michael C. Frank} \\ Department of Psychology \\  Stanford University  \\  450 Serra Mall \\ Stanford, CA 94305 \\ \textit{mcfrank@stanford.edu}}
...
Inspired by this prior work, our goal is to understand the relationship between developmental changes in how children draw visual concepts and their representations of these visual concepts.  In particular, we hypothesize that children’s drawings become more recognizable in part because children learn the distinctive features of particular categories that set them apart from other similar objects (Figure \ref{fig:drawingExamples}). If so, we would expect an increase in the distinctive visual features depicted in children's drawings (e.g., long ears to depict bunnies) that are not simply due to improvements in children's motor control abilities. However, this goal poses several methodological challenges to overcome.

First, it requires a principled and generalizable approach to encoding the high-level visual properties of drawings that expose the extent to which they contain category-diagnostic information [@FanCommon2018]. This approach stands in contrast to previous approaches, which have relied upon provisional criteria specific to each study (e.g., handles for mugs) [e.g., @barrett1976symbolism; @goodenough1963goodenough], which limited their ability to make detailed predictions on new tasks or datasets. We meet this challenge by capitalizing on recent work validating the use of deep convolutional neural network (DCNN) models as a general basis for measuring the high-level visual information that drives recognition in images [@yamins2014performance]. As these models have been trained to recognize objects in photographs, units in higher layers tend to respond to the visaul features that are most useful for distinguishing between different object categories. Here, we analyze activations of a DCNN in responses to sketches, as previous work has validated both the overlap in high-level features between sparse drawings and photographs of objects and the use of these model feautres for classifying drawings [@FanCommon2018; @long2018drawings]. In particular, we evaluate whether children include distinctive features in their drawings by assessing how well these visual features can be used to identify the category (e.g., dog, bird) that children were intending to draw.
...
Thus, the current paper provides a substantial advance over prior work on children's drawings [@long2018drawings] in several ways. Our larger dataset allows us to take novel analytic approaches that characterize how discriminable drawings of different categories are at each age and to directly measure changes in visual distinctiveness. Furthermore, in contrast to prior work, we develop automated procedures for scoring children's visuomotor abilities and the visual distinctiveness of their drawings, and apply these procedures across a much wider range of ages, categories, and individuals. 
...
# Acknowledgements
We thank the San Jose Children's Discovery Museum for their collaboration and for hosting the drawing station where these data were collected. We are grateful to members of the Stanford Language and Cognition lab for their feedback. This work was funded by an NSF SPRF-FR Grant #1714726 to BLL and a Jacobs Foundation Fellowship to MCF. 
