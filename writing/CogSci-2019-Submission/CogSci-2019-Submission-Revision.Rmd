...
classification_data <- read.csv('../../data/cogsci_2019/classification-outputs/classification-outputs-041919-balanced_C_0.1_2691.csv') %>%
  as.tibble() %>%
...
To compute these error terms, we applied an image registration algorithm, Airlab [@sandkuhler2018], to align each tracing to the target shape, yielding an affine transformation matrix minimizing the pixel-wise correlation distance between the aligned tracing, $T$, and the target shape, $S$: $Loss_{NCC} = - \frac{\sum S \cdot T - \sum E(S) E(T)}{N \sum Var(S) Var(T)}$, where $N$ is the number of pixels in both images. 
...
The shape error was defined by the cross-correlation loss between the transformed tracing and the target shape. The spatial error was defined by three sources of error: location, orientation, and size error, derived by decomposing the affine transformation matrix into translation, rotation, and scaling components. In sum, this procedure yielded four values for each tracing, one value representing the shape error (i.e., the pixel-wise correlation distance) and three values representing the spatial error (i.e., magnitude of translation, rotation, scaling components). 
