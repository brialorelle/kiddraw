...
In sum, our paper provides an advance over our prior work investigating developmental change in drawing behavior [@long2018drawings] in three ways: first, we describe automated procedures for characterizing the category-level distinctiveness inherent to children's drawings for a wide range of ages; second, we build a platform to continually crowdsource large numbers of children's drawings under consistent conditions; and third, we develop a scalable approach to analyzing concurrent changes in visuomotor control using a tracing task. 
