...
basis for representing shape similarity -- including the presence of
diagnostic object parts (e.g., legs, handles) -- and a basis from which
object identity can be easily derived (Kubilius et al., 2016). We then
use these high-level features to evaluate how similar the
...
explain why adults are more accurate in recognizing their drawings.
...
resampling of the 27 object categories.
...
VGG-19. These values are plotted relative to the a comparable
correlation between two samples of adult drawings, where similarity is
high across all layers. We found that children's and adults' RDMs were
more similar than younger children's and adult's RDMs across all layers,
and that similarity plateaued in middle to high-level layers (see Figure
\ref{fig:layerWise}). However, due to the small size of our current
dataset, we caution against strong interpretations of particular
