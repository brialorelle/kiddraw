...
technique that dates back 60,000 years (D. L. Hoffmann et al., 2018;
Pike et al., 2012), well before the emergence of symbolic writing
systems, and is practiced in many cultures.
...
in each layer of the model. In DCNN models, the earliest layers tend to
capture local image statistics, including edges and textures -- whether
there is more horizontal or vertical information -- and intermediate
layers capture mid-level features, including curvature (Yamins et al.,
2014). Thus, these analyses reveal how similar children and adult's
drawings are in terms of these more basic statistics -- e.g., if both
children and adult's drawings of apples are curvier than their drawings
of phones.
...
shoes). Each of these 27x27 representational dissimilarity matrices
...
resampling of the 27 object categories. To estimate the noise ceiling,
we repeated this same procedure with an equivalently sized, random
sample of adult drawings.
...
VGG-19. These values are plotted relative to a comparable correlation
between two samples of adult drawings, representing the noise ceiling --
where similarity is high across all layers. We found that children's and
adults' RDMs were more similar than younger children's and adult's RDMs
across all layers, and that similarity plateaued in middle to high-level
convolutional layers (see Figure \ref{fig:layerWise}). However, due to
the small size of our current dataset, we caution against strong
interpretations of particular layer-wise estimates. Nonetheless, adults'
and children's drawings were dissimilar in pixel space for both age
groups (adults vs.~older children, r = .07; vs.~younger children, r =
.07).
...
frogs, and flowers) elicited the most similar representations, evident
in a cluster in the middle of the RDM. While there are some intuitive
category pairings (e.g., drawings of cars are somewhat similar to trains
and buses), other clusters seem less intuitive at first blush
(carrot--chair--ice cream--fork)---yet note that all four of these
...
these same object categories. Children's and adults' drawings appear to
share many of the perceptual features useful for object recognition.
