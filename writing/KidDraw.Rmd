...
ggplot(ms, aes(x = category, y = mean, col = age_group)) +
  ...
  scale_color_viridis(discrete=TRUE, begin=0, end=.4, "Age group", labels=c("3-6 yrs.", "7-10 yrs."))
...
```{r layerWise, fig.env = "figure", fig.pos = "H", fig.align='center', fig.width=3, fig.height=4, set.cap.width=T, num.cols.cap=1, fig.label="headcam", fig.cap = "Spearman's correlation between representational dissimilarity matrices (RDMs) between drawings produced by adults and younger children (light blue line) and between adults and older children (dark blue line) at each layer of VGG-19--the first five convolutional layers and the last two fully connected layers. Correlations between two random samples of adult drawings are shown in green, estimating the noise ceiling. Error bars represent standard error of the mean obtained by a jacknife re-sampling procedure (see Methods)."}
ggplot(cohortSim, aes(x = LayerNum, y = correlation, col=cohortType)) +
 ...
 theme(legend.direction = "horizontal", legend.position="bottom") +
 scale_color_viridis(discrete=TRUE, begin=0, end=.8, direction = -1, "Cohort Type",    labels=c("Adult--Adult","OlderKids--Adults","YoungerKids--Adults"))
```  
## Results
