...
```{r covDescriptives, fig.env="figure*", fig.height=2, fig.width=7, fig.pos = "h", fig.align = "center", fig.cap = "R plot" }
p1<-ggplot(joint, aes(x = num_strokes, y=meanCorrect, col=age)) +
  geom_jitter(alpha=.3) +
  geom_smooth(aes(group = 1), method="loess",span=3, alpha=.1) +
  ylim(0, 1) +
  labs(y = "Prop. recognized", x = "Number of strokes") 

p2<-ggplot(joint, aes(x = mean_intensity, y=meanCorrect, col=age)) +
  geom_jitter(alpha=.3) +
  geom_smooth(aes(group = 1), method="loess",span=3, alpha=.1) +
  labs(y = "", x = "Amount of 'ink' used") 

p3<-ggplot(joint, aes(x = draw_duration, y=meanCorrect, col=age)) +
  geom_jitter(alpha=.3) +
  geom_smooth(aes(group = 1), method="loess",span=3, alpha=.1) +
...
(Intercept) & 0.861 & 0.321 & 2.680 & 0.007 \\ 
  ...
  Drawing duration & 0.338 & 0.109 & 3.105 & 0.002 \\ 
  Amount of ink & 0.014 & 0.080 & 0.179 & 0.858 \\ 
  Number of strokes & -0.289 & 0.098 & -2.959 & 0.003 \\ 
   \hline
\end{tabular}
\caption{Model coefficients of a GLMM predicting the recognziability of each  drawing.} 
\end{table}
...
Overal, we found that the capacity to quickly produce graphical representations that communicate object category information is highly developed by middle childhood. 


An obvious future direction for this work is to understand the contribution of children's motor control abilities in producing graphical representations. In other words, to what degree are drawings made by older children more recognizable simply because older children have finer motor control abilities?
