...
To complement the children's drawing dataset, we additionally obtained a random sample of 100 adult drawings from each of the drawn categories above from the Google Quickdraw dataset (https://quickdraw.withgoogle.com/data).

### Deep convolutional neural network model
We used a standard, pre-trained implementation of the popular VGG-19 architecture [@simonyan2014very] to extract features from sketches at layers across several depths in the network. Specifically, we analyzed feature activations in the first five pooling layers, as well as the first two fully-connected layers. Prior to analysis, we cropped all sketch images to contain only the sketch, applied uniform padding (10px), and rescaled them to the same size (3x224x224). Each image elicits a pattern of feature activations at every layer in the model, each pattern being equivalent to a vector in a feature space with the same number of dimensions as units in that layer.
