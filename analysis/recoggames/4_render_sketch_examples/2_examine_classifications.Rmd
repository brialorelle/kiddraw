...
meta_animalgame <- read.csv('compiled_classifications/classification-outputs-animalgame_C_0.1_560.csv') %>%
  as.tibble() %>%
...
meta_vehiclegame <- read.csv('compiled_classifications/classification-outputs-vehiclegame_C_0.1_460.csv') %>%
  as.tibble() %>%
  mutate(session_id = paste('cdm_',session_id,sep="")) %>%
  mutate(age_numeric = age) %>%
  mutate(age = paste('age',age,sep="")) %>%
...
meta <- meta_animalgame %>%
  full_join(meta_vehiclegame)
```
...
thres_by_category <- meta %>%
  ...
  gather(Percentile, Threshold, '1':'0.15')

##
age_thres = 0
dir_name = 'sketch_examples/distinctiveness_by_category_correct_and_incorrect_15_start'
dir.create(dir_name)
```
...
for (this_category in categories){
  ...
  for (upper in upper_thresholds) {
    ...
    subset <- meta %>%
      ...
      mutate(image_path = paste('sketches/',exp,'/',image_name,sep="")) %>%
