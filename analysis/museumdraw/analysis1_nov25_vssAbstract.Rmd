...
Full random effects structure (session_id = subj, category = prompt) with extra fixed effects for number of strokes, draw duration, and avg intensity (e.g., amount of ink)
```{r}
## Full model first
# Can't include random slopes (1+category|sessionId) since not every subject drew every thing
...
fullModelReport=lmer(normCorrect ~ age + category + mean_intensity + draw_duration + (1|session_id), joint, REML = TRUE)
...
## Alternative solution - generalized logistic mixed-effects models
Get convergence warnings here even with minimal models -- ugh. -- commented out.
``` {r}
