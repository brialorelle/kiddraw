...
```{r}
## Render out new copy of filtered dataset for mturk experiments (no practice shapes)
file.copy(from = fs::path(filtered_drawings_objects$fullPath), fs::path(filtered_dataset_path,filtered_drawings_objects$fileNameShortNew))

file.rename(from = file.path(filtered_dataset_path,filtered_drawings_objects$fileNameShort),to=file.path(filtered_dataset_path, filtered_drawings_objects$fileNameShortNew))
