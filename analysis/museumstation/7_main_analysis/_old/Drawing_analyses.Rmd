...
cor_by_session <- d %>%
  ...
  # filter(age_numeric >2) %>%
  dplyr::summarize(mean = mean(correct_or_not), num_drawings = n(), tracing = avg_tracing_rating) %>%
  group_by(age_numeric)
base_size_chosen=10 
...
cor_by_session <- d %>%
  group_by(age_numeric,session_id) %>%
  dplyr::summarize(mean = mean(correct_or_not), num_drawings = n(), tracing = mean(avg_tracing_rating)) %>%
  filter(!is.na(tracing))

base_size_chosen=10
smooth_alpha=.01

ggplot(cor_by_session, aes(tracing,mean*100, col=age_numeric, size=num_drawings)) +  
  geom_jitter(alpha=.02, width=.1, height=5) +
  geom_smooth(alpha=smooth_alpha, color='grey', span=10) +
  theme_few(base_size = base_size_chosen) + 
  labs(x='Tracing estimate', y='Drawing classification accuracy') +
