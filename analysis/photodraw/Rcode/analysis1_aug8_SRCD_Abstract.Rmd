...
ggplot(ratingConfusions, 
  ...
  ylab("True Category") + 
  xlab("Rated as") + 
  scale_fill_viridis(limits = c(0, 1),option="magma") + 
...
ggplot(data = predicted_df, aes(x=age, y=lmer_predictions)) +
  ...
  geom_jitter(data = corbySketch, aes(x=age, y=propCorrect, col=age))

## Model prediction lines overliad on subject-level CI data
ggplot(data = predicted_df, aes(x=age, y=lmer_predictions)) +
  facet_grid(~condition) +
  scale_color_viridis()  +
  stat_smooth(method="lm", col='grey') +
