...
<!-- ggplot(joined,aes(x=category,y=avg_stroke_length, col=age))+ -->
...
mod <- glmer(correct ~ age*condition + draw_duration + num_strokes + (1 | sessionId) + (1 | category),  data = joined,  family = "binomial")
...
joined$condition = factor(joined$condition, levels = c("S", "W", "P"))
joined$age 
mod <- glmer(correct ~ age*condition + draw_duration + num_strokes + (1 | sessionId) + (1 | category),  data = joined,  family = "binomial", control=glmerControl(optCtrl=list(maxfun=20000),optimizer=c("bobyqa")))
model_summary = round(summary(mod)$coef,3);
kable(model_summary, digits = 3)
