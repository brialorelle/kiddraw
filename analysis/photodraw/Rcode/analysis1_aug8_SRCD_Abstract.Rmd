...
corbyCondition <- r %>%
  ...
  mutate(meanCorrect=mean(correct)) %>% # calculate mean correct (subject-level average correct)
...
## How do num strokes / draw duration / avg_stroke_legth relate to recognizability?
```{r} 
corByItem <- joined %>%
...
ggplot(corByItem, aes(x = avg_stroke_length, y = avgCorrect, col=condition)) +
    geom_jitter(width = .1, alpha = .5) +
    facet_grid(~age)
```
...
# scale age
corbySketch$age_sc = scale(corbySketch$age, scale = FALSE)

## Adding random slopes on items causes the model to fail to converge, only keeping random intercepts.
model_lmer <- lmer(propCorrect ~ age_sc*condition + (1 | sessionId) + (1 | category),  data = corbySketch)

```

## Secondary analysis: add draw_duration and num_strokes as covariates

```{r}


joined$condition = factor(joined$condition, levels = c("S", "W", "P")) # make sure the conditions are in the right order here
joined$category = as.factor(joined$category)

mod_covariates <- glmer(correct ~ scale(age)*condition + draw_duration + num_strokes + (condition | sessionId) + (1 | category),  data = joined,  family = "binomial", control=glmerControl(optCtrl=list(maxfun=20000),optimizer=c("bobyqa")))
model_summary_covariates = round(summary(mod_covariates)$coef,3);
kable(model_summary_covariates, digits = 3)
...
model_lmer <- lmer(propCorrect ~ age_group*condition + (1 | sessionId) + (1 | category),  data = corbySketch)
summary(model_lmer)
```
