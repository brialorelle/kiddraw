...
mod <- glmer(correct ~ scale(age)*condition + (condition | sessionId) + (1 | category),  data = r,  family = "binomial",   control=glmerControl(optCtrl=list(maxfun=20000),optimizer=c("bobyqa")))
...
mod <- glmer(correct ~ scale(age)*condition + draw_duration + num_strokes + (condition | sessionId) + (1 | category),  data = joined,  family = "binomial", control=glmerControl(optCtrl=list(maxfun=20000),optimizer=c("bobyqa")))
